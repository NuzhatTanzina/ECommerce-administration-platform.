...
export class SellerController {
    ...
    async UploadBookImage(@UploadedFile() myfileobj: Express.Multer.File): Promise<any>
    {
    ...
    @Get('/book/search')
    @UseGuards(SessionGuard)
    async searchBooks(@Query('criteria') criteria: string, @Query('value') value: string, @Session() session): Promise<BookEntity[]> {
    if (!criteria || !value) {
    ...
    @UseGuards(SessionGuard)
    @UsePipes(new ValidationPipe())
    async UpdateProfileInfo(@Body() updated_data:SellerDTO, @Session() session): Promise<any>{
    ...
    @UsePipes(new ValidationPipe())
    async UpdatePassword(@Body() updated_Data:UpdatePasswordDTO, @Session() session): Promise<any>{
    ...
    @UseGuards(SessionGuard)
    @UsePipes(new ValidationPipe())
    async UpdateOrderStatus(@Param('id', ParseIntPipe) id:number, @Body() updated_data:OrderDTO): Promise<any>{
    ...
    @UseGuards(SessionGuard)
    async GetTotalNumberOfPendingOrderCurrentMonth(@Session() session): Promise<number>{
